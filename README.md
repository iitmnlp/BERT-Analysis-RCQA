...
* The IG scores are stored in a folder called ```ig_scores```, in files named in the format ```importance_scores_ig_layer_number.npy``` 
* Embeddings for the first 200 datapoints are stored in a folder called ```embs```, in files named in the format ```emb_enclayer_layer_number.npy``` 
