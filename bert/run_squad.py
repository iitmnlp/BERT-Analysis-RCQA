...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  model = modeling.BertModel(
      ...
      use_one_hot_embeddings=use_one_hot_embeddings,scope="bert")
  ...
  grads_1=tf.gradients(tf.reduce_max(start_probs,axis=1),all_encoder_layers[cnt])
  tf.logging.info("grads_1 : %s",str(grads_1))

  grads_2=tf.gradients(tf.reduce_max(end_probs,axis=1),all_encoder_layers[cnt])
...
#                                   ["unique_id", "start_logits", "end_logits", "importance_scores"])
RawResult = collections.namedtuple("RawResult",
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
                      ...
                      output_nbest_file, output_null_log_odds_file, output_impscore_file,output_conicity_file,output_emb_file):
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    #doc_words[example.qas_id]=example.doc_tokens
  ...
  #importance_scores_dict={'words' : doc_words, 'imp_scores' : imp_scores}
  ...
  for i in range(50) : 
    save_embs_for_tsne[all_ids[i]]=all_embs[all_ids[i]]
  ...
  for i in range(len(all_ids)) : 
    ct_id=all_ids[i]
    if i%1000==0 : 
      tf.logging.info("Conicity loop : %d",i)
    ct_embs=all_embs[ct_id]
    ct_embs_norm=np.linalg.norm(ct_embs,axis=1)+1e-14
    if i==0 or i==1 : 
      tf.logging.info("%s",ct_embs_norm)
    mean_ct_embs=np.mean(ct_embs,axis=0) # emb_size
    mean_ct_embs_norm=np.linalg.norm(mean_ct_embs)+1e-14
    atm_cos=np.inner(ct_embs,mean_ct_embs)/(ct_embs_norm*mean_ct_embs_norm)
    conicity=np.mean(atm_cos)
    conicities.append(conicity)
  conicities=np.array(conicities)
  np.save(output_conicity_file,conicities)
  tf.logging.info("Conicities saved!")
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    # steps.
    all_results = []
    # importance_scores = collections.OrderedDict() 
    for result in estimator.predict(
      ...
      #importance_scores=integrated_gradients
      #if FLAGS.span_or_enclayer=='enclayer' : 
    ...
    output_impscore_file=os.path.join(FLAGS.output_dir,"importance_scores_ig_"+str(FLAGS.do_integrated_grad)+".npy") # 27
    output_conicity_file=os.path.join(FLAGS.output_dir+"/../conicity","conicity_span_"+str(FLAGS.do_integrated_grad)+".npy") # 29
    output_emb_file=os.path.join(FLAGS.output_dir+"/../embs","emb_span_"+str(FLAGS.do_integrated_grad)+".npy") # 299
    
    write_predictions(eval_examples, eval_features, all_results,
                      FLAGS.n_best_size, FLAGS.max_answer_length,
                      FLAGS.do_lower_case, output_prediction_file,
                      output_nbest_file, output_null_log_odds_file,
                      output_impscore_file,output_conicity_file,output_emb_file)
